...
- `backend/app/agents/runtime/` 保存 Runtime、Store 契约与 SSE；Run、节点、事件和 Tool 调用由 PostgreSQL Store 持久化，当前进程只保存 SSE Subscriber Queue。
- `backend/app/rag/` 保存 Schema、Loader、Splitter、Embedding、ChromaDB Store、检索和生命周期；只有真实 Chroma 命中可标记为 `CHROMA_HYBRID`，失败时明确回退。
