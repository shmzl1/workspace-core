...
禁飞区算法只能由人工负责人维护。AI 不得复制、重写、模拟、绕过禁飞区核心算法。

禁飞区保持纯 Python，不依赖 FastAPI、SQLAlchemy、LangGraph、ChromaDB、HTTP 客户端、数据库连接或 LLM。禁飞区只接收结构化输入，只输出结构化结果。
...
- 当前任务属于哪个目录。
- 已读取根级 `AGENTS.md` 和最近目录下的 `AGENTS.md`。
- 不触碰 AI 禁飞区。
- 不新增第二套后端。
- 不绕过 Service。
