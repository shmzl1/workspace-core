...
- `agents/runtime/`：RunStore、公开调度兼容入口、节点公共失败处理与 SSE。
- `agents/shared/`：事件、状态、来源、Guardrail 与模型网关契约。
- `agents/workflows/`：招聘、员工服务、薪资预审的工作流契约；招聘六个业务 Agent 文件承载真实 LangGraph Node。
...
flowchart TB
    ...
    Match --> Interview[面试评估 Agent]
    Interview --> Review
    Review -->|无待人工审核节点| Report[HR 最终报告]
    Review -->|存在待人工审核节点| Wait[结束本次执行并等待既有审核接口]
    Wait -->|全部审核通过后从报告入口恢复| Report
    Report --> HumanDecision[HR 人工决定]
```

招聘完整入口严格顺序执行 `recruitment_strategy → resume_parser → job_match → interview_evaluation → decision_review`。无真实结构化面试反馈时仍进入 `interview_evaluation` Node，由节点保存 `SKIPPED` 并发布既有事件。完整入口与审核后的报告入口共用同一 Graph 定义；Graph State 不保存 Store、Session、ApplicationContainer、Tool 或 ModelGateway，运行时依赖通过编译闭包注入。代码存在，待本地人工验收。
...
招聘策略 Agent 中互不依赖的模型叙述增强与企业知识检索并行执行。策略增强和 HR 最终报告增强默认关闭模型深度思考、限制叙述输出长度，并分别设置 25 秒和 35 秒节点内预算；超出预算时继续使用已经生成的确定性计划或报告，标记为 `RULE_BASED_FALLBACK`。该延迟控制代码存在，待本地人工验收。
...
OpenAI-compatible Chat Client、Provider-selected Embedding Client、ChromaDB Persistent Collection 和本地演示知识摄取代码存在，待本地人工验收。火山方舟 Provider 对每个 Chunk 单独调用 `/embeddings/multimodal`；配置禁用或调用失败时分别回退到确定性叙述和 `LOCAL_HYBRID_FALLBACK`，不虚构模型输出或 RAG 命中。
...
员工智能助手相关代码存在，待本地人工验收。其调用链为：

```text
Vue AssistantView
  -> POST /api/v1/assistant/chat
  -> AssistantService
  -> ApplicationContainer.model_gateway
```

- Assistant API 只负责理解当前问题、脱敏摘要、最多 12 条最近消息及上一轮结果字段描述，返回 `QUERY_DATA`、`ANSWER_FROM_RESULT`、`CHAT` 或 `UNKNOWN`；接口要求 JWT，但不会把 User、JWT、Employee ID 或结果字段值传给模型。
- `QUERY_DATA` 下的 `LEAVE`、`PAYROLL`、`POLICY` 由前端固定映射到已有业务 API，真实数据仍经过原 Service、Repository、权限和审计链路。业务接口成功后，前端在当前页面内存生成包含真实 `value` 的本地 Facts；传给 Assistant 的转换结果只保留 `key`、`label`、`unit`、`value_type`。
- `ANSWER_FROM_RESULT` 只让模型选择 `READ`、`CONFIRM`、`COMPARE` 或 `EXPLAIN` 以及可用 fact key。前端读取本地真实值确定性生成最终回答，不直接采用模型回复作为业务事实，不重复调用业务接口，也不重新展示结果卡片。
- Assistant 模块与招聘 Agent Runtime 完全隔离，不创建 Run、Snapshot 或 AgentEvent，不写 Agent Run Store，不使用 SSE、Workflow 或 Tool。两者只共享 `ApplicationContainer.model_gateway` 模型基础设施。
- 会话只保存在当前页面内，使用最长 4000 字符的脱敏摘要和最多 12 条、单条最长 1000 字符的最近消息；真实结果值不进入摘要或最近消息。页面刷新不恢复，清空对话会清除摘要、消息和本地结果上下文。模型不可用时仅增加必要的数字确认兜底，其余仍使用既有业务关键词规则。
