...
当前阶段为 `SPRINT_2_3_DETERMINISTIC_INTERMEDIATE`，执行招聘策略、简历解析、岗位匹配、决策审查和 HR 最终报告，下一阶段为 `LLM_RAG_INTEGRATION`。面试评估只能读取真实结构化面试数据，无数据时以 `STRUCTURED_INTERVIEW_FEEDBACK_NOT_AVAILABLE` 标记为 `SKIPPED`；决策审查仍继续运行并记录面试数据缺失。决策审查不得静默修改确定性评分，最终报告不拥有录用、淘汰、排期确认或薪资确认权，最终决定由 HR 完成。相关代码存在，待本地人工验收。
...
Sprint 2.3 继续使用既有三个 Run API、JWT、`agent.hr.use`、Run 所有者隔离、进程内 RunStore 与 SSE 裸 `AgentEvent`。Runner 只处理已脱离数据库 Session 的 Pydantic 上下文，Runtime 不访问 Repository 或 `human_only`。`NEEDS_REVIEW` 是合法业务结果；只要结构化结果正常生成，工作流仍可结束为 `COMPLETED`。代码存在，待本地人工验收。

当前未接入真实大模型、LangGraph、LangChain、ChromaDB、Embedding、外部 HTTP 模型调用或真实本地知识库。企业知识继续使用 `LOCAL_HYBRID_FALLBACK`，不虚构 RAG 命中。
