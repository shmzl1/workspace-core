...
Sprint 2.1 当前数据流为：`API -> RecruitmentRunContextService -> RecruitmentService/Repository` 校验真实上下文，然后由进程内 Runtime 生成招聘策略计划并通过 SSE 推送事件。Run 归创建用户所有，只存在于当前进程，重启后丢失。
