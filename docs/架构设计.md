...
- Assistant API 只负责理解当前问题、脱敏摘要与最多 12 条最近消息，返回固定意图、年月、政策关键词、普通回复和更新摘要；接口要求 JWT，但不会把 User、JWT 或 Employee ID 传给模型。
- `LEAVE`、`PAYROLL`、`POLICY` 由前端固定映射到已有业务 API，真实数据仍经过原 Service、Repository、权限和审计链路。模型不访问数据库、不返回真实薪资、假期余额或政策正文。
...
- 会话只保存在当前页面内，使用最长 4000 字符的脱敏摘要和最多 12 条、单条最长 1000 字符的最近消息；页面刷新不恢复，清空对话会清除全部上下文。模型不可用时只对既有业务意图使用本地规则兜底。
