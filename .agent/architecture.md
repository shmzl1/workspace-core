...
TalentFlow 智聘中枢采用 Vue Web 管理端、微信小程序员工端、Gradio 内部调试台和一套 FastAPI 后端。后端采用模块化单体。Sprint 2.1 当前使用有界进程内 Runtime 和 SSE 运行招聘策略 Agent；LangGraph、LLM、RAG 和 Gradio Agent 执行仍未接入。
...
正式招聘链路为“企业招聘目标 → 招聘策略 Agent → 简历解析 / 岗位匹配 / 面试评估 → 决策审查 → HR 最终报告 → HR 人工决定”。当前只执行招聘策略节点；其余节点只建立契约，不能解释为真实 Agent 能力。
