...
TalentFlow 智聘中枢采用 Vue Web 管理端、微信小程序员工端、Gradio 内部调试台和一套 FastAPI 模块化单体后端。PostgreSQL Agent Run 持久化、六节点 LangGraph 编排、OpenAI-compatible 策略/报告增强、ChromaDB 企业知识检索、SSE 和前端恢复代码存在，待本地人工验收；真实面试评价 Agent 和 Gradio Agent 执行为计划中。
...
- 员工智能助手使用独立调用链：`Vue AssistantView -> POST /api/v1/assistant/chat -> AssistantService -> ApplicationContainer.model_gateway`。接口区分 `QUERY_DATA`、`ANSWER_FROM_RESULT`、`CHAT`、`UNKNOWN`；模型只选择业务领域、参数、结果操作和可用 fact key。
- `QUERY_DATA` 下的 `LEAVE`、`PAYROLL`、`POLICY` 仍由前端固定映射到已有本人假期、本人薪资、月度考勤和政策 API。真实接口结果在当前页面内存转换为本地 Facts；发送给模型的只有字段 `key`、`label`、`unit`、`value_type`，绝不包含 `value`。
- `ANSWER_FROM_RESULT` 由前端根据本地真实值执行 `READ`、`CONFIRM`、`COMPARE`、`EXPLAIN` 并确定性生成最终回答，不重复调用业务接口、不重复展示业务卡片，也不将模型回复作为数值事实。
- 页面上下文采用最长 4000 字符的脱敏摘要、最多 12 条且单条最长 1000 字符的最近消息窗口；真实结果值不写入摘要、最近消息或 localStorage。清空页面会话会同时清空摘要、消息窗口、最近业务意图和本地结果上下文，刷新页面不恢复会话。
- `backend/app/modules/assistant/` 不导入招聘 Runtime、Workflow、Tool 或 `modules/agent_runtime`，不创建 Run、Snapshot、AgentEvent，不写 Agent Run Store，不使用 SSE。员工智能助手与招聘 Agent 唯一共享的能力是应用容器中的 `model_gateway`。
- ModelGateway 禁用、误配置、不可用或输出非法时，Assistant API 返回安全错误；前端先对“是/是不是/有/还剩 + 数字 + 单位”使用本地真实结果完成有限确认，再对 `LEAVE`、`PAYROLL`、`POLICY` 使用既有关键词和最近业务意图兜底，不伪造模型回复。

## 长期多 Agent 架构与目录职责

正式招聘链路为“企业招聘目标 → 招聘策略 Agent → 简历解析 → 岗位匹配 → 面试评估（无真实结构化评价时跳过）→ 决策审查 → HR 最终报告 → HR 人工决定”。六个业务阶段均为独立 LangGraph Node；决策审查后存在待人工审核节点时结束本次 Graph 执行，全部审核通过后由同一 Graph 的报告入口进入 `hr_report`。确定性核心、可选 LLM 叙述增强、真实 RAG 与 LangGraph 编排代码存在，待本地人工验收；真实面试评价 Agent 为计划中。

- `agents/runtime/`：Run Store 契约、调度兼容入口、节点公共失败处理与 SSE；PostgreSQL 实现位于 `modules/agent_runtime/`，内存只保存 Subscriber Queue。
- `agents/shared/`：状态、事件、来源引用、Guardrail 与模型网关契约。
- `agents/workflows/`：领域状态、节点元数据、六个可执行业务节点和纯 Tool 编排函数；Graph State 只保存 `run_id`、入口和人工审核路由标记。
