...
- `rag/`：本地 Loader、稳定 Splitter、OpenAI-compatible Embedding、ChromaDB Store、混合检索、引用和生命周期；失败时由招聘 Service 明确回退为 `LOCAL_HYBRID_FALLBACK`。
...
- 最新迁移：`backend/alembic/versions/0004_agent_runtime.py`，revision 为 `0004_agent_runtime`，down_revision 为 `0003_add_leave_requests`。
