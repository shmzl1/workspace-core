...
- 员工智能助手使用独立调用链：`Vue AssistantView -> POST /api/v1/assistant/chat -> AssistantService -> ApplicationContainer.model_gateway`。接口只进行自然语言理解、固定意图识别、年月与政策关键词解析、普通聊天回复和脱敏摘要更新。
- `LEAVE`、`PAYROLL`、`POLICY` 仍由前端固定映射到已有本人假期、本人薪资、月度考勤和政策 API；模型不返回真实业务数据、不控制接口地址、不访问 Repository。
- 页面上下文采用最长 4000 字符的脱敏摘要、最多 12 条且单条最长 1000 字符的最近消息窗口，不保存完整业务响应、薪资金额、具体假期余额或完整政策正文；清空页面会话会同时清空摘要、消息窗口和最近业务意图，刷新页面不恢复会话。
...
- ModelGateway 禁用、误配置、不可用或输出非法时，Assistant API 返回安全错误；前端仅对 `LEAVE`、`PAYROLL`、`POLICY` 使用既有本地关键词规则和轻量最近业务意图兜底，不伪造普通聊天或模型成功状态。
